...
import { DeleteComment } from "@/actions/profile/comment";
...
export default function ResponsiveDialog({ id }: props) {
  ...
  const onDeleteComment = async (CmtId: string) => {
    await DeleteComment(CmtId).then((res) => {
      if (res?.succses) {
        toast.success(res.succses);
        setOpen(false);
      }
      if (res?.error) {
    ...
    <React.Fragment>
      ...
      >
        ...
        <DialogActions className=" dark:border-white dark:bg-black dark:text-white">
          <Button onClick={() => onDeleteComment(id)} autoFocus>
            Có
          </Button>
