...
export const ModalBody = ({
  ...
  className,
  open,
  onClose, // Thêm thuộc tính onClose
}: {
  children: ReactNode;
  className?: string;
  open: boolean;
  onClose: () => void; // Định nghĩa kiểu cho thuộc tính onClose
}) => {
  useEffect(() => {
    // Đặt thuộc tính overflow của body khi modal mở và đóng
    if (open) {
    ...
    // Cleanup: Đảm bảo đặt lại overflow khi modal bị hủy
    return () => {
      document.body.style.overflow = "auto";
    };
  }, [open]);
  ...
  useOutsideClick(modalRef, () => onClose()); // Sử dụng onClose
    ...
    <AnimatePresence>
      {open && (
        ...
        >
          ...
          >
            <CloseIcon onClose={onClose} /> {/* Truyền onClose */}
            {children}
          </motion.div>
        </motion.div>
      )}
...
const CloseIcon = ({ onClose }: { onClose: () => void }) => {
  return (
    <button
      onClick={() => onClose()} // Gọi onClose khi nhấp vào
